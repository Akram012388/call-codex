...
Native macOS visibility decision: `visibility: "macos_app"` requires the host runtime to expose `CODEX_NATIVE_APP_SERVER_URL=ws://127.0.0.1:<port>`, with optional `CODEX_NATIVE_APP_SERVER_AUTH_TOKEN_FILE`. `CALL_CODEX_APP_SERVER_URL` is only a dev/test override and is treated as macOS-visible only when `CALL_CODEX_APP_SERVER_BACKEND=macos_app` is also set. Without the native bridge, visible worker creation fails clearly; background mode can still use managed loopback workers. `call_bridge_status` diagnoses this contract from inside the plugin MCP process before visible-worker smoke tests.
